...
public class UserDetailsServiceImpl implements UserDetailsService {
	...
	public UserDetailsServiceImpl(MemberMapper memberMapper) {
		...
		log.info("[+] UserDetailsServiceImpl created ");
	}
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		List<SimpleGrantedAuthority> authorities = member.getAuthorities().stream()
				.map(auth -> new SimpleGrantedAuthority(auth.name())).toList();

		return User.withUsername(username).password(member.getPassword()).authorities(authorities).build();
	}
